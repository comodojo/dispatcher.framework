...
comodojo kernel (release 1.0 on the way).Also XML and JSON encoder/decoder are
the same.
...
the second one contains real service logic (and it's located in services folder).
Files could be merged into one (that means that configuration could be hardcoded
in service logic, although it's not recommended).

In addition, framework comes with a URL-to-service router (index.php located in
root folder) that can centralize requests to different services, redirecting or
masquerading (cloaking) results in agreement to what declared in routing table
(contained in configs/router-config.php).

It can also route/cloak requests to not explicitly declared services (autoroute)
using the service file name.
...
Please refer to single configuration files for parameters explanation:
 - main-config.php
 - router-config.php
...
Defining a new service is quite easy.
...
- logFile (STRING): name of the relative log file. If false and trace is enabled,
  service will log in default $logFile="global.log"
...
- accessControlAllowOrigin (STRING of comma-separated domains): define valid origin
  in CORS requests
...
You can yse simple HTTP methods pointers ($this->get, $this->put, ...) or, if you
don't need to discriminate http methods or simply you want some more flexible,
customized logic, you can use the "logic" method, invoked on each request, no matter
the type, if specific method pointer is not declared.

You can also copy the file structure from examples and:

 - include right service configuration file
 - write the service logic (that's the most important part) according to your needs
 
Service logic methods should not return any value BUT they HAVE TO POPULATE two class
variables:
...
SimpleDataRestDispatcher support two levels of caching (when request are via GET
method):
...
Having said this, router cache can ensure best performances but first request can be
heavy to process and, in addition, embedded curl http call could be slow.
In the other hand, client caching is free, but lots client in short unit of time
will produce also heavy traffic and lots of service processing cicles.

That means that, if you need this feature, server caching (with or without client
cache) is recomended for services with lots of requests or few changes during day
(long ttl); client cache is suggested in any condition but for services with lots
of changes in short units of time.

A NOTE ON ACCESS CONTROL
========================

CORS access control is a really simple way to enable cross origin request.
The current version of dispatcher implements only the Access-Control-Allow-Origin
header, and it checks the HTTP-ORIGIN header to handle requests.
...
- if access-control value is a list of domains, comma separated, framework will explode
  values and match HTTP-ORIGIN just like previous point
...
All example service are located in services folder.
 ...
 - example_database_based_service.php: interact with local database via HTTP methods.
 
 - example_external_service.php: output lorem ipsum text from loripsum.net;
 
 - example_internal_service.php: output local file.
   
 - example_hello_world.php: simple hello-world service.
 
 - example_hello_world_cors.php: simple hello-world service with CORS check.
 
 - example_hello_world_logic.php: simple hello-world service with logic() method.
 
 - example_hello_world_post_only.php: simple hello-world service with POST support only.
...
All tests are located in test folder.
 ...
 - test_php_functions.php: checks if your php installation are compliant
   to functions, extensions and grants required by framework.

 - test_services.php: tests services directly, without router.

 - test_router.php: tests router and it's extra functionalities (like caching).

 - test_xhr.html: tests framework using XHR javascript requests (with dojotookit).
